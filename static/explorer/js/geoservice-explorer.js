...
function fieldBuilderTemplate(){
	...
	var compare = {
		"LIKE" : "is like ",
		"&amp;gt;" : "is greater than (>)",
		"&amp;gte;" : "is greater or equals than (>=)",
		"&amp;lt;" : "is less than(<)",
		"&amp;lte;" : "is less or equals than (<=)"
	};
	...
	for(var k in geoservice_schemes[$("#collections").val()]){
		...
		if(k=="loc"){
			...
			if(typeof(geoservice_schemes[$("#collections").val()]["loc"])=="object"){
				loopObj(geoservice_schemes[$("#collections").val()]["loc"],tmp,k);
			}
		}else{
...
function setContent(geoservice_schemes,schema,col){
	...
	$("#querybuilder").empty();
	$('<form onsubmit="getData();return false;" action="#"><label for="fulltext">Text search: </label><input type="text" id="fulltext" /><br /><a href="#" class="action" onclick="return fieldBuilderTemplate()"><span class="icon add">&nbsp;</span> Add a field to the query</a><div class="clear"></div><input type="submit" id="execquery" value="execute query" /></form>').appendTo("#querybuilder");
...
function getData(page,sort,order){
	...
	var aux;
	if($("#fulltext").val()){
		params.push("&fulltext=",encodeURIComponent($("#fulltext").val()));
		z++;
	}
	...
	$.getJSON("/"+collection+"/search?"+params.join("")+"&cb=?", function(data){
		if(data){
			...
			tmp.push("<a href='",kml,"' target='_blank'>KML</a> | ");
			tmp.push("<a href='",url_init,"&format=csv' target='_blank'>CSV</a> | ");
			tmp.push("<a href='",url_init,"&format=xls' target='_blank'>XLS</a> | ");
			tmp.push("<a href='",url_init,"&format=html' target='_blank'>HTML</a> | ");
			tmp.push("<a href='",url_init,"' target='_blank'>JSON</a> | ");
